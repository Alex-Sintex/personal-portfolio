 ...
 *
 * @author Kevin Alexis
 * @version 2.4.17 - Modified
 */

if ("undefined" == typeof jQuery) throw new Error("KAs website requires jQuery");
! function (i) {
   "use strict";
   ...
   function (i) {
      ...
      s.prototype.init = function () {
         ...
         i(document).on("click", function (event) {
            if (
               !i(event.target).closest(self.element).length &&
               !i(event.target).is(self.element)
            ) {
               self.collapse();
            }
         });
